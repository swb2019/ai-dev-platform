...
Need a fresh fork for testing? Run the commands below (requires a PAT with `delete_repo` scope) and they will delete the old fork, recreate it privately, reclone it, and sync it. Replace `your-user` with your GitHub handle:
...
if (Test-Path C:\dev\ai-dev-platform) {
    Remove-Item -Recurse -Force C:\dev\ai-dev-platform
}
